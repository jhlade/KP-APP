...
% ------------------------------------------------------------------------------
% Třída dokumentu je nastavena přímo v app.tex (musí být fyzicky první příkaz
% v hlavním dokumentu), tento soubor obsahuje jen balíčky a makra preambule
...
\fancypagestyle{StyleAppendix} {
	...
	\rhead{}
	\cfoot{\thepage}
	\rfoot{\textcolor{gray}{\textit{Přílohy}}}
}
...
\makeatletter
\def\obrazek#1#2{%
  \@ifnextchar[{\obrazek@i{#1}{#2}}{\obrazek@i{#1}{#2}[]}}
\def\obrazek@i#1#2[#3]{%
  \begin{figure}[!ht]
	\begin{center}
		\ifx\relax#3\relax
