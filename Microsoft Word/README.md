**Verze pro Microsoft Word (desktop)**

⚠️ Nepoužívejte v&nbsp;jiných programech, není pro ně stavěná. Zároveň není plně podporována
ve webovém prostředí Office 365 a nedoporučuje se používat ji na tabletech (dochází
k zanášení matoucích stylů).

[➡️ Přímo ke stažení zde jako `kp-app.dotx`](./kp-app.dotx?raw=true).<br>
[➡️ Varianta s pomocnými makry `kp-app.dotm`](./kp-app.dotm?raw=true).
...
Varianta [`.dotm`](./kp-app.dotm?raw=true) s pomocnými makry přidává do panelu nástrojů
menu `Abs. práce` možnost pomocí snadných tlačítek přidat nebo schovat prázdné stránky
na začátku a na konci dokumentu (k tisku) a podobně možnost skrýt nebo vytvořit stránku
pro nepovinné *poděkování* hned za *Obsahem*. Aby bylo možné této výhody využít,
je třeba při otevření šablony spouštění maker povolit.

![Povolení maker](https://github.com/jhlade/KP-APP/blob/assets/editory/msword-makra.webp?raw=true)

> 🔎 Makra neobsahují žádný škodlivý kód. V [`build`](./build) je možné najít původní
zdrojový kód a nástroje použité k sestavení šablony.